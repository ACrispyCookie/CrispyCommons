...
public class TimeToLiveManager {
    ...
    private void startGlobalTask(@NotNull Set<OfflinePlayer> viewers, @NotNull Runnable runnable, long duration) {
        ...
        if (duration < 0)
            return;
        tasks.put(null, Bukkit.getScheduler().scheduleSyncDelayedTask(SpigotCrispyCommons.getInstance().getBukkitPlugin(), () -> {
            runnable.run();
    ...
    private void startPlayerTask(@NotNull UUID uuid, @NotNull Runnable runnable, long duration) {
        ...
        if (duration < 0)
            return;
        tasks.put(uuid, Bukkit.getScheduler().scheduleSyncDelayedTask(SpigotCrispyCommons.getInstance().getBukkitPlugin(), () -> {
            runnable.run();
