...
public abstract class AbstractDynamicElement<T, K> extends AbstractElement<T, K> implements DynamicElement<T, K> {
    ...
    private static class ElementTaskHandler {
        ...
        private static void addElement(AbstractDynamicElement<?, ?> element) {
            Bukkit.getScheduler().runTaskLater(SpigotCrispyCommons.getInstance().getBukkitPlugin(), () -> {
        ...
        private static void startNewTask(int period) {
            tasks.put(period, Bukkit.getScheduler().runTaskTimer(SpigotCrispyCommons.getInstance().getBukkitPlugin(), () -> elements.get(period).forEach(AbstractDynamicElement::runUpdate), 0, period));
        }
    }
}
